...
export default function ProductPage() {
  ...
  useEffect(() => {
    params.id ? (
      getProdect(params.id as string).then((res) => setPrudect(res))
    ) : (
      <h1>Loading...</h1>
    );
  }, [params.id]);
    ...
    <Container>
      <div className="flex flex-col items-center ">
        <div className="shadow-md w-500">
          ...
          <div className="p-3">
            <p className="mb-4 text-gray-700">{Prodect?.description}</p>
            {getCountCards(parseInt(params.id as string)) === 0 ? (
              <Button
                onClick={() => AppInCountCards(parseInt(params.id as string))}
              ...
              <div className="flex border-2 bg-green-800 p-3">
                <DoneIcon />
